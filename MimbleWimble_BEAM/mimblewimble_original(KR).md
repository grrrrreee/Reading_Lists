...
이는 "블록체인"이라고 불리우는 공개 데이터베이스에 저장함으로써 완성된다. 진정으로 확인하고자 하는 사람은 이 상태를 전체 다운로드하고 모든 거래를 재실행시켜 보아야한다. 한편 대부분의 거래들은 실제 최종 상태에 영향을 미치지 않는다. (그들은 나중에 파괴된 산출물을 만든다)

이 글을 쓰고있는 현재(2016년 7월), 블록체인에서 약 1억5천만 건의 거래가 있었고 이들을 실행하면 4백만개의 사용되지 않은 출력값이(UTXO) 생성된다. 

감사자가 출력값 자체에 대한 데이터만 검사하면 된다면 좋겠지만, 출력값이 이전 출력값 체인의 끝에 있는 경우에만 유효하고, 다음에 각각 서명해야 하기 때문에 이것은 불가능하다. 즉, 최종 상태의 유효성을 확인하기 위해서는 전체 블록체인(blockchain)을 검증해야 한다. 

여기에 이들 거래는 암호화된 원자성이라, 각 거래에 어떤 출력값이 투입되고 생성되는지가 명확하다. 그 결과로 나타나는 "거래그래프"는 많은 정보를 드러내며, 하위 계층을 감시하고 통제하는 사업 모델을 가진 많은 회사들의 분석을 받는다. 이로 인해, 사람들의 privacy 를 침해하고 심지어 위험할 수도 있다. 

이에 대한 몇 가지 해결책이 제시되었다. 그렉 멕스월이 양을 암호화하는 방법을 찾았다, 거래의 그래프는 faceless 하지만 그 총량이 옳다는 것을 증명해낼 수 있다.[1] 멕스웰 박사는 CoinJoin 이라는 것을 또한 만들었다, CoinJoin 은 비트코인에서 사용되는 방법이다. 거래를 interactively 하게 합쳐서 거래 그래프를 혼란스럽게 한다. 니콜라스 반 사베르하겐은 거래 항목을 가리는 시스템을 개발했고, 훨씬 더 나아가 거래 그래프 역시 가린다. (사용자 상호 작용이 필요 없을 뿐만 아니라)[3]. 후에 셴 노에더는 두 가지 접근법을 결합하여 맥스웰의 "기밀 거래"와 판 사버하겐의 가림을 달성했다. 
...
OWAS는 그 거래를 블록으로 묶는 좋은 아이디어를 가지고 있었다. 상상해보면 블록 간 결합할 수 있으므로(아마도 일부 glue 데이터와 함께) 출력이 생성 및 파괴될 때 마치 존재하지 않는 것처럼 동일하다. 그러면 전체 체인의 유효성을 검사하고, 사용자는 돈이 입력된 경우에만 알 수 있다 시스템(비트코인 또는 모네로 또는 페그인 각 블록의 사이드 체인) 및 최종 비사용 출력, 나머지는 제거될 수 있으며, 잊혀진 
...
블록체인에서 모든 사용자의 정보에 대해 이야기하는 것을 막기 위해 사용되기 때문에 나는 내 창작물을 Mimblewimble이라고 부른다. [7]

<h3>Confidential Transactions and OWAS </h3>

우리가 가장 먼저 해야 할 일은 비트코인 스크립트를 제거하는 것이다. 슬프지만, 너무 강력해서 일반 스크립트를 사용하여 거래를 병합하는 것은 불가능하다. 우리는 맥스웰 박사의 비밀거래가 생산물의 소비를 승인하기에 충분하고 또한 상호 작용 없이 결합 거래를 할 수 있다는 것을 증명할 것이다. 이는 사실상 OWAS와 동일하며, 중계 노드가 거래 수수료를 일부 부담하거나 수신자가 거래 수수료를 변경할 수 있도록 한다. 비트코인이 할 수 없는 부가적인 것들, 우리는 공짜로 얻을 수 있다. 
...
이 출력에 첨부된 rangeproof는 v가 0에서 2^64 사이에 있음을 증명하여 사용자가 블라인딩을 이용하여 오버플로 공격을 할 수 없도록 한다. 
