...
이 글을 쓰고있는 현재(2016년 7월), 블록체인에서 약 1억5천만 건의 거래가 있었고 이들을 실행한 결과 4백만개의 사용되지 않은 출력값이(UTXO) 생성되었다.

감사자가 출력값 자체에 대한 데이터만 검사할 필요가 있다면 더 좋겠지만, 출력값이 이전 출력값 체인의 끝에 있는 경우에만 유효하고, 다음에 각각 서명해야 하기 때문에 이것은 불가능하다. 즉, 최종 상태의 유효성을 확인하기 위해서는 전체 블록체인(blockchain)을 검증해야 한다. 
...
이에 대한 몇 가지 해결책이 제시되었다. 그렉 멕스웰이 암호화를 발견했다. 이 액수는 액면 그대로지만 액수가 맞는지 확인할 수 있다. 맥스웰 박사는 또한 비트코인 사용자들이 상호작용적으로 거래를 결합하여 거래 그래프를 혼란스럽게 하는 시스템인 Coinjoin을 생산했다. 니콜라스 반 사베르하겐은 거래 항목을 블라인드하기 위한 시스템을 개발했고, 훨씬 더 나아가 거래 그래프를 흐리게 했다(사용자 상호 작용이 필요 없을 뿐만 아니라). 후에 셴 노에더는 두 가지 접근법을 결합하여 맥스웰의 "기밀 거래"와 판 사버하겐의 암영을 얻었다. 

닥터 맥스웰의 Coinjoin은 상호작용을 필요로 하는 문제를 가지고 있다. 위안 호라스 무튼 박사는 자유롭게 거래를 병합할 수 있게 함으로써 이것을 고쳤으나, 잠재적으로 더 느리고 신뢰하기 어려운 페어링 기반의 암호학을 사용할 필요가 있었다. 그는 이것을 "일방 종합 서명"(OWAS)이라고 불렀다. OWAS는 그 거래를 블록으로 묶는 좋은 아이디어를 가지고 있었다. 
...
이 출력에 첨부된 범위 방지 기능은 v는 0에서 2^64 사이에 있음을 증명하여 사용자가 블라인딩을 이용하여 오버플로 공격을 할 수 없도록 한다. 

거래를 검증하기 위해, 검증자는 모든 산출물에 f*H(여기에 명시적으로 주어진 거래 수수료)를 더하고 모든 투입 약속을 차감한다. 결과는 0이어야 하며, 이는 전체적으로는 어떠한 금액도 중간에 생성되거나 파괴되지 않았다는 것을 증명한다. 
